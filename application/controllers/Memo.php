...
class Memo extends CORE_Controller {
    ...
    function transaction($txn = null, $filter_value= null) {
        switch ($txn) {
            ...
            case 'memo_report':
                ...
                $data['data']=$this->Memorandum_model->get_listofmemos(null,null,null,null,$emp_memo_id);

                $file_name=$data['emp_info'][0]->full_name.' (Memo # '.$data['data'][0]->memo_number.')';
                $pdfFilePath = $file_name.".pdf"; //generate filename base on id
                $pdf = $this->m_pdf->load(); //pass the instance of the mpdf class
                $content=$this->load->view('template/memo_list_html',$data,TRUE); //load the template
            ...
            case 'read_memo':
                ...
                $m_memo->is_read = 1;
                $m_memo->modify($emp_memo_id);

                $response['memo_count'] = count($m_memo->get_listofmemos($this->session->employee_id,null,null,1));
            ...
            case 'listofmemo':
                ...
                $employee_id = $this->session->employee_id;
                $response['data']=$m_memo->get_listofmemos($employee_id,$from_date,$to_date);
                echo json_encode($response);
                break;
