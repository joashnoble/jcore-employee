...
class Profile extends CORE_Controller {
    ...
    public function index()
    {
        ...
        $data['_modal']=$this->load->view('template/elements/change_password_modal',$data1,TRUE);
    ...
    function transaction($txn = null, $filter_value= null) {
        switch ($txn) {
            ...
            case 'update_password':
                ...
                $current_pass = $this->input->post('current_password', TRUE);
                ...
                $check_password = $this->Employee_model->get_emp_pass($employee_id,$current_pass);

                if (count($check_password) > 0){

                    $m_account = $this->Employee_account_model;
                    $m_account->employee_pwd = sha1($new_pass);
                    $m_account->modify($employee_account_id);

                    $response['title']='Success!';
                    $response['stat']='success';
                    $response['msg']=' Your password is successfully updated.';
